...
function resolveConfig(api: OpenClawPluginApi) {
  ...
  const userId =
    ...
    "";
  if (!accessToken)
    throw new Error("Instagram access token not configured. Set INSTAGRAM_ACCESSTOKEN env var.");
...
async function uploadToR2(
  ...
  const {
    ...
    filename,
    contentType,
  } = await readOwnedLocalImage(localPath, dataDir);
  ...
  if (!accessKeyId || !secretAccessKey || !bucket || !publicUrl || !endpoint) {
    throw new Error(
      "R2 credentials not configured. Set R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET, R2_PUBLIC_URL, R2_ENDPOINT env vars.",
    );
  }
...
const ToolSchema = Type.Object(
  {
    ...
    image_urls: Type.Array(Type.String(), {
      description:
        "Array of image URLs (1-10). Local /images/ paths are auto-uploaded. 1 image = single post, 2+ = carousel.",
      minItems: 1,
      maxItems: 10,
    }),
...
export function createInstagramPublishTool(api: OpenClawPluginApi) {
  return {
    ...
    description:
      "Publish single image or carousel to Instagram. Pass image_urls array (1=single, 2+=carousel). Local /images/ paths are auto-uploaded to public URL. Max 2200 char caption.",
    ...
    async execute(_toolCallId: string, rawParams: Record<string, unknown>) {
      ...
      if (caption.length > 2200)
        throw new Error(`Caption exceeds 2200 char limit (${caption.length} chars).`);
        ...
        if (publicUrls.length === 1) {
          ...
          for (const url of publicUrls) {
            const childResp = await fetch(`${API_BASE}/${userId}/media`, {
              ...
              headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "Idempotency-Key": `${attempt.idempotencyKey}-${childIds.length}`,
              },
              body: new URLSearchParams({
                image_url: url,
