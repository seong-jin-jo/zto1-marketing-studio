 ...
 * claimToken 으로 lease 소유권까지 확인한다. 토큰 누락은 언제나 거절한다.
 */
export function verifyPublishable(
  ...
  if (!token) {
    return {
      ...
      message: "발행 예약 소유권을 확인할 claimToken 이 필요합니다",
    };
  }
...
/** 공급자 결과 기록과 release가 쓰는 소유권 검사. 채널 상태는 별도로 판정한다. */
export function verifyClaimOwnership(
  post: ClaimablePost | null | undefined,
  claimToken: string | null | undefined,
  options: { now?: Date; allowExpired?: boolean } = {},
): PublishVerdict {
  if (!post) return { ok: false, reason: "post-canceled", message: "작업물이 큐에 없습니다" };
  if (!claimToken) return { ok: false, reason: "claim-required", message: "claimToken 이 필요합니다" };
  if (!post.claim) return { ok: false, reason: "claim-missing", message: "이 작업물의 발행 예약이 해제됐습니다" };
  if (post.claim.token !== claimToken) {
    return { ok: false, reason: "claim-mismatch", message: "다른 워커가 이 작업물을 가져갔습니다" };
  }
  if (!options.allowExpired && !isClaimActive(post, options.now ?? new Date())) {
    return { ok: false, reason: "claim-expired", message: "발행 예약 시간이 만료됐습니다" };
  }
  return { ok: true };
}

export type PublishAttemptVerdict =
  | { ok: true; idempotencyKey: string }
  | { ok: false; reason: Extract<PublishVerdict, { ok: false }>["reason"] | "result-recovery-required"; message: string };

/**
 * pending에서 publishing으로 바꾸는 원자 경계에서만 공급자 호출 허가를 만든다.
 * publishing은 취소 성공 응답과 외부 호출이 엇갈리는 창을 닫고, 중단된 호출은 복구 대상으로 남긴다.
 */
export function beginPublishAttempt(
  post: ClaimablePost | null | undefined,
  channel: ChannelKey,
  options: { claimToken: string | null | undefined; idempotencyKey: string; now?: Date },
): PublishAttemptVerdict {
  const current = post?.channels?.[channel];
  if (current?.status === "publishing") {
    return {
      ok: false,
      reason: "result-recovery-required",
      message: "이 채널은 공급자 결과 확인이 필요해 다시 발행할 수 없습니다",
    };
  }
  const verdict = verifyPublishable(post, channel, {
  ...
  if (!current) {
    return { ok: false, reason: "channel-not-pending", message: `${channel} 채널 대기 상태가 없습니다` };
  }
  const now = options.now ?? new Date();
  current.status = "publishing";
  current.publishAttempt = {
    claimToken: options.claimToken as string,
