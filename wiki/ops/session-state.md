...
기본 흐름 11/11, Studio v1 14/14다. 전체 src design lint는 기존 카드 테마 정의 파일을 hex
리터럴로 집계했고 변경 화면 경로는 위반 0이다.

Studio v1 검증기의 기존 후보 거절 누락을 현재 R27 계약에 맞게 수정했다. 다음 행동은 갭 정정본,
구현현황, QA tracker와 검증기 변경을 범위 한정 커밋하고 QA 단계에 넘기는 것이다. 운영 배포는
