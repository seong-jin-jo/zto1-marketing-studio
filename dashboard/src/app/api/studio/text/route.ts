...
출력은 JSON만(다른 텍스트 없이):
{
 ...
 "image_prompt": "히어로 이미지 생성용 영문 프롬프트. 영어 1~2문장. 사람의 손·물건·공간 같은 실물 피사체 하나와 구도·빛·재질만 묘사한다. 한국어·브랜드명·제품명 금지. 화면·모니터·문서·간판·아이콘·차트·말풍선처럼 글자가 놓일 물체는 절대 등장시키지 않는다(부정어로 적지 말고 그냥 다른 피사체를 골라라). 좋은 예: 'A hand pouring coffee into a ceramic cup on a wooden table, soft morning light.' / 'Two people shaking hands in a bright office, warm natural light.' 나쁜 예(쓰지 말 것): 'A laptop screen showing a dashboard with charts and icons.' / 'A shop sign with a logo and text.'"
}`;
  try {
