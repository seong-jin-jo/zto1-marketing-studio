 ...
 */
export async function probeVideo(filePath: string): Promise<{ width: number; height: number; durationSec: number }> {
...
// 생성·발행 라우트와 같은 이유로 실패를 502 로 돌려주지 않는다. 우리 앞의 리버스 프록시가
// 502 본문을 자기 HTML 오류 페이지로 갈아치워, 우리가 쓴 이유가 사용자 화면까지 못 간다.
const WORK_REFUSED = 200;
...
export async function POST(request: Request) {
  ...
  if (!inputPath) {
    return Response.json({ ok: false, error: "자막을 넣을 영상을 찾지 못했습니다. 생성실에서 영상을 다시 만들어 주세요." }, { status: 400 });
  }
  ...
  try {
    ...
    const missingTool = /ENOENT/.test(message);
    return Response.json({
      ok: false,
      code: missingTool ? "ENCODER_MISSING" : "SUBTITLE_BURN_FAILED",
      error: missingTool
        ? "이 서버에 영상 편집기가 설치되어 있지 않아 자막을 넣지 못했습니다."
        : "자막을 넣는 중에 영상 처리가 끝나지 않았습니다. 잠시 후 다시 시도해 주세요.",
    }, { status: WORK_REFUSED });
  }
