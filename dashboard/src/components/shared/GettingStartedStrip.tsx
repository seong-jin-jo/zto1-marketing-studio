...
export function GettingStartedStrip({
  connectedCount: controlledConnectedCount,
  learningFilled,
  learningTotal = 7,
  onOpenLearning,
}: {
  connectedCount?: number;
  learningFilled?: number;
  ...
  const learningIncomplete = onOpenLearning && learningFilled !== undefined && learningFilled < learningTotal;
  ...
  if (done >= STEPS.length && !learningIncomplete) return null;
    ...
    <section className="mb-pad-inset" aria-label="시작 안내" data-start-strip>
      <div className="flex min-h-control-touch items-center gap-stack rounded-surface border border-accent/30 bg-accent-soft px-stack py-stack-tight text-caption text-accent">
        ...
        {/* 단추가 늘 "채널 연결하기" 였다. 채널을 이미 붙인 사람에게는 할 일이 아니다.
            지금 남은 칸으로 바로 데려간다. */}
        {learningIncomplete ? (
          <button type="button" data-testid="getting-started-learning" onClick={onOpenLearning} className="inline-flex min-h-control-touch shrink-0 items-center rounded-control bg-accent px-stack text-caption font-semibold text-accent-fg">
            학습 정보 채우기
          </button>
        ) : (
