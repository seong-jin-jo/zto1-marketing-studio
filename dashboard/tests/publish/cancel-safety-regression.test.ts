...
describe('발행기 도구 계약 — get_approved/update_channel 이 가드를 경유한다', () => {
  const toolSrc = () =>
    fs.readFileSync(
      path.resolve(__dirname, '../../../openclaw/extensions/threads-queue/src/threads-queue-tool.ts'),
      'utf-8',
    );
  ...
  it('update_channel 이 published/failed 기록 전에 claim 소유권과 공급자 결과를 확인한다', () => {
    const src = toolSrc();
    const updateChannel = src.slice(src.indexOf('case "update_channel"'));
    expect(updateChannel).toMatch(/verifyClaimOwnership/);
    expect(updateChannel).toMatch(/provider-result-missing/);
  });
...
describe('Codex 교차 리뷰 잔여 결함 — 경계 케이스', () => {
  ...
  it('MAJOR 3: claim 이 없는 레거시 경로도 토큰 없이 통과하지 못한다', () => {
    const post: ClaimablePost = { id: 'p', status: 'approved', channels: { threads: { status: 'pending' } } };
    const verdict = verifyPublishable(post, 'threads', {});
    expect(verdict.ok).toBe(false);
    expect(verdict.ok === false && verdict.reason).toBe('claim-required');
  });
  ...
  it('MAJOR 7: 운영 예약 발행 경로가 모든 작업 공간의 outbox를 매 실행마다 drain한다', () => {
    const route = fs.readFileSync(
      path.resolve(__dirname, '../../src/app/api/schedule/publish-due/route.ts'),
      'utf-8',
    );
    expect(route).toContain('listQueueMirrorOutboxTenantIds');
    expect(route).toContain('drainQueueMirrorOutbox');
    const postStart = route.indexOf('export async function POST');
    const firstDrain = route.indexOf('drainQueueMirrorOutbox()', postStart);
    const firstScheduleProcessing = route.indexOf('processTenant(', postStart);
    expect(firstDrain).toBeGreaterThan(postStart);
    expect(firstDrain).toBeLessThan(firstScheduleProcessing);
  });

  it('MAJOR 8: outbox 상한을 넘어도 미수렴 항목을 하나도 버리지 않는다', async () => {
    vi.doMock('@/lib/queue-store', async () => {
      const actual = await vi.importActual<typeof import('@/lib/queue-store')>('@/lib/queue-store');
      return {
        ...actual,
    ...
    expect(file?.entries).toHaveLength(511);
    expect(file?.droppedCount ?? 0).toBe(0);
    expect(readQueueMirrorOutbox()).toHaveLength(511);
    vi.doUnmock('@/lib/queue-store');
  });
