# OSMU 네 방 기본 흐름 QA 재검증 v11

> STAMP | line: osmu | 생성: 2026-09-15 02:27 KST | model: gpt-codex/gpt-5 | agent: qa-verifier | skill: qa | 근거: v63 프로토타입, 확정 요구 정본, 사업 좌표, localhost 실요청, Playwright 원본 캡처 | 고민: 생성 엔진 실행환경 결함은 고치되 기능 통과를 디자인 승인이나 운영 배포 통과로 확대하지 않았다.

한 줄 결론: 현재 localhost의 네 방 기능은 생성 엔진 실행경로를 복구한 뒤 기본 API 11/11, 단면 4/4, 4개 폭 방 화면 20/20과 성과실에서 생성실 복귀 5/5, Studio v1 14/14로 통과했다. v63과 현재 16개 화면의 구조가 달라 디자인 정합과 제품 전체 QA는 NG다.

검증실패 보고: 등급=제품 전체 QA NG, 사유=v63 디자인 정합 실패와 운영 배포 미검증, 출고여부=localhost 기능 회귀 증거로만 출고하며 QA 승인과 배포 근거로는 출고하지 않음.

## 요청 추적

| 요청번호 | 요청 요지 | 테스트번호 | 판정 | 증거 |
|---|---|---|---|---|
| R08, R166, R172 | 생성실부터 성과실까지 네 방 관통 | FLOW-API-V11 | 수정 후 PASS | 최초 후보 생성은 `spawn_failed`와 후보 0장으로 중단. CLI 사용자 설치 경로 복구 뒤 지정 작업 공간에서 localhost 기본 흐름 11/11. 후보 3장, 편집, 삭제와 복원, 발행 큐 HTTP 201, 성과 제안 3건과 생성실 재인계를 관찰 |
| R08, R19, R207 | 네 방 렌더와 390, 768, 1024, 1440 실제 이동 | FLOW-UI-V11 | 기능 PASS, 디자인 NG | 단면 4/4, 20개 방 화면과 성과실에서 생성실 복귀 5/5. 가로 넘침, 가린 모달, 탐색 차단, 401, 콘솔 오류 0건 |
| R27, R168 | Studio v1 생성과 무료 다시 만들기 경계 | STUDIO-V1-V11 | PASS | localhost 실요청 14/14 |
| R104 | 검증용 고객 자격증명을 남기지 않음 | FLOW-PROBE-CLEANUP-V11 | PASS | 모든 검증 뒤 활성 `qa-four-room-*` 토큰 0건 |
| R201 | 사족, 긴 대시와 영문 단추 라벨 금지 | FLOW-COPY-V11 | PASS | 전체 `V69-COPY` 계약을 포함한 회귀 통과. 현재 캡처의 단추는 한국어이며 UI 직접 문자열 긴 대시 금지 테스트도 통과 |
| R193, R205, R206 | 승인 프로토타입의 발행실과 공통 셸 계승 | DESIGN-V11 | NG | v63과 현재 공통 셸, 주축, 요소 순서, 열 수, 담당 패널, 글꼴 단계와 버튼 위계가 다름 |
| R01부터 R207 중 이번 범위 밖 | 확정 요구 전건 누락 방지 | REQ-ALL-V11 | 이월 | 요구 정본을 유지하고 이번 네 방 기능 PASS에 포함하지 않음 |
...
| backend build와 test | PASS | 별도 Spring backend 없음. Next Route Handler 포함 Vitest 353파일, 2,293건 PASS, 조건부 3건 제외 |
...
| health | PASS | 최종 localhost:3456 listener PID 42353, HTTP 200, DB up, 26ms |
| seed | PASS | `apply-schema.sh --seed`로 schema, 지정 작업 공간, RLS, 확장과 역할을 멱등 적용 |
| 주요 API | 수정 후 PASS | `verify-basic-flow-e2e.mjs` 최종 11/11. build 뒤 재기동한 서버에서도 11/11 재확인 |
...
| 상위 QA 품질 게이트 | FAIL | `verify-agent-quality.sh` 종료 코드 2. 운영 배포 환경 접촉 증거 0건으로 반려 |
| 운영 배포 | 미검증 | 실제 배포 버전, 외부 계정 발행과 운영 성과 회수는 확인하지 않음 |

원본은 `logs/diff/osmu-four-room-flow-20260915-0216/captures/`다. 동적 URL은 `/studio?room=create`, `/studio?room=edit`, `/studio?room=publish`, `/performance`다. `observations.json`은 방 관찰 20건과 복귀 5건, 콘솔 오류 0건, 401 URL 0건을 담는다. 최종 `dashboard/src`, `dashboard/scripts`, `dashboard/tests` 합성 SHA-256은 `f9919ac18a62566b2b461a0f77de21c2bd848af0853664bb812e70f5222b2ecf`다.

## 발견과 수정

최초 두 번의 `verify-basic-flow-e2e.mjs`는 health HTTP 200과 DB up인데도 첫 후보 생성에서 `STUDIO_LLM_PROVIDER_UNAVAILABLE`, 후보 0장으로 종료했다. 서버 관찰 로그의 직접 원인은 `shared_ai_generation_execution_failed`의 `spawn_failed`였다. 01:26부터 떠 있던 Next 프로세스의 PATH에는 Claude CLI 설치 위치 `/Users/sj/.local/bin`이 없었고 `CLAUDE_BIN`도 없었다.

`dashboard/src/lib/anthropic.ts`의 단일 CLI 실행 경계가 명시 환경값, 사용자 기본 설치 경로, PATH 순서로 실행 파일을 찾도록 고쳤다. 수정 커밋은 `629f056d`, 회귀 테스트 커밋은 `957a8225`다. 수정 전 신규 회귀 1건이 실패했고 수정 후 대상 테스트 23건, 전체 2,293건과 localhost 11단계가 통과했다. API 계약, DB 스키마와 화면 구조는 바꾸지 않았다.
...
- 추가와 변경: Claude CLI 실행 파일 탐색과 전용 회귀 테스트만 추가했다. 화면, API 계약과 DB 스키마는 변경하지 않았다.
...
이 결론이 틀렸다면 가장 그럴듯한 이유는 사용자 설치 경로를 macOS 한 위치로만 가정해 다른 호스트에서 다시 실패할 수 있다는 점이다. 대응: 명시 `CLAUDE_BIN`을 최우선으로 유지하고 두 사용자 기본 경로 뒤에도 기존 PATH 탐색을 보존했다. 이번 호스트에서는 수정본을 새 프로세스로 구동한 뒤 11단계 실요청이 두 번 연속 통과했다.
...
SOURCES/MODEL: gpt-codex/gpt-5 | `docs/design/prototypes/legacy-prototype-20260912/prototype/openclaw-auto-4room-v63.html` | `docs/_archive/legacy-20260912/requests/회장-확정-요구사항-대장.md` | `wiki/거버넌스/요청.md` | `wiki/2-product/build/사업좌표-OSMU와-ZERO-ONE.md` | `docs/eng-design/fdd-legacy-20260912/fdd/test-plan-r02-v1.0.0-opus.md` | `docs/plan/persona.md` | `docs/plan/one-thing.md` | `docs/design/README.md` | `docs/design/captures/manifest.json` | `logs/diff/osmu-four-room-flow-20260915-0216/captures/` | https://playwright.dev/docs/actionability | https://playwright.dev/docs/emulation | https://playwright.dev/docs/test-snapshots
