# OSMU 네 방 기본 흐름 QA 재검증 v10

> STAMP | line: osmu | 생성: 2026-09-14 22:35 KST | model: gpt-codex/gpt-5 | agent: qa-verifier | skill: qa | 근거: v63 프로토타입, 확정 요구 정본, 사업 좌표, localhost 실요청, Playwright 원본 캡처 | 고민: 기능 회귀 통과를 승인 시안 정합이나 운영 배포 통과로 확대하지 않고 세 판정을 분리했다.

한 줄 결론: 현재 localhost의 네 방 기능은 기본 API 11/11, 단면 4/4, 4개 폭의 방 화면 20/20과 성과실에서 생성실 복귀 5/5, Studio v1 14/14로 통과했다. v63과 현재 16개 화면의 구조가 달라 디자인 정합과 제품 전체 QA는 NG다.
...
|---|---|---|---|---|
| R08, R166, R172 | 생성실부터 성과실까지 네 방 관통 | FLOW-API-V10 | PASS | 지정 작업 공간에서 localhost 기본 흐름 11/11. 후보 3장, 편집, 삭제와 복원, 발행 큐 HTTP 201, 성과 제안 3건과 생성실 재인계를 관찰 |
| R08, R19, R207 | 네 방 렌더와 390, 768, 1024, 1440 실제 이동 | FLOW-UI-V10 | 기능 PASS, 디자인 NG | 단면 4/4, 20개 방 화면과 성과실에서 생성실 복귀 5/5. 가로 넘침, 가린 모달, 탐색 차단, 401, 콘솔 오류 0건 |
| R27, R168 | Studio v1 생성과 무료 다시 만들기 경계 | STUDIO-V1-V10 | PASS | localhost 실요청 14/14 |
| R104 | 검증용 고객 자격증명을 남기지 않음 | FLOW-PROBE-CLEANUP-V10 | PASS | 모든 검증 뒤 활성 `qa-four-room-*` 토큰 0건 |
| R201 | 사족과 영문 단추 라벨 금지 | FLOW-COPY-V10 | PASS | 원본 캡처 20장에서 영문 단추 라벨을 발견하지 못함 |
| R193, R205, R206 | 승인 프로토타입의 발행실과 공통 셸 계승 | DESIGN-V10 | NG | v63과 현재 공통 셸, 주축, 요소 순서, 열 수, 담당 패널, 버튼 위계가 다름 |
| R01부터 R207 중 이번 범위 밖 | 확정 요구 전건 누락 방지 | REQ-ALL-V10 | 이월 | 요구 정본을 유지하고 이번 네 방 기능 PASS에 포함하지 않음 |
...
| health | PASS | localhost:3456 listener PID 53664, 최종 HTTP 200, DB up, 24ms |
...
원본은 `logs/diff/osmu-four-room-flow-20260914-2208/captures/`다. 동적 URL은 `/studio?room=create`, `/studio?room=edit`, `/studio?room=publish`, `/performance`다. `observations.json`은 방 관찰 20건과 복귀 5건, 콘솔 오류 0건, 401 URL 0건을 담는다. 최종 `dashboard/src`, `dashboard/scripts`, `dashboard/tests` 합성 SHA-256은 `21d3abaddecfb5e98fd3540fc5cdc94d502b4198251ee3aabf1bd1c08363318c`이며 세 경로에 이번 QA의 소스 변경은 없다.

## 실행 환경 오염 분리

첫 `npm run test`는 같은 셸에서 `.env.local`을 내보낸 상태라 인증 단위 테스트 16파일 59건이 실패했다. 이는 브라우저와 API 검증에 필요한 실제 자격증명이 격리 테스트의 기본 환경을 덮은 실행자 오염이다. 이 결과는 제품 결함 판정에서 제외하고 원본 `vitest.log`에 보존했다. 새 셸에서 문서에 적힌 공식 명령 `npm run test`를 그대로 다시 실행해 351파일과 2,291건 통과, 조건부 3건 제외를 확인했고 권위 로그는 `vitest-clean.log`다. 제품 코드 수정은 없으며 다음 QA도 실자격증명 셸과 단위 테스트 셸을 분리해야 한다.
...
- 추가와 변경: 제품 소스, API 계약, DB 스키마를 바꾸지 않았다. 새 회귀에서 끊긴 곳이 없어 QA 증거와 문서만 추가했다.
...
이 결론이 틀렸다면 가장 그럴듯한 이유는 이전 v9와 제품 소스가 같다는 사실만으로 16개 화면의 디자인 불일치를 답습했을 가능성이다. 대응: 이번에 새로 만든 20장 중 390과 1440 생성실, 1024 편집실, 발행실, 성과실을 v63 동일 폭 원본과 직접 열어 다시 대조했다. 셸, 주축, 요소 순서, 열 수와 담당 패널 차이를 재관찰했다.

## 벤치마크 적용

- Playwright 시각 비교 원칙에서 동일 환경과 일관된 기준 PNG 대조를 차용했다: https://playwright.dev/docs/next/test-snapshots
...
SKILLS_USED: `qa`, localhost 실제 탐색, 회귀 우선, 시각 대조, 실행 환경 오염 분리와 최종 판정에 사용 / SKILLS_SKIPPED: 없음
...
SOURCES/MODEL: gpt-codex/gpt-5 | `docs/design/prototypes/legacy-prototype-20260912/prototype/openclaw-auto-4room-v63.html` | `docs/_archive/legacy-20260912/requests/회장-확정-요구사항-대장.md` | `wiki/거버넌스/요청.md` | `wiki/2-product/build/사업좌표-OSMU와-ZERO-ONE.md` | `docs/eng-design/fdd-legacy-20260912/fdd/test-plan-r02-v1.0.0-opus.md` | `docs/plan/persona.md` | `docs/plan/one-thing.md` | `docs/design/README.md` | `docs/design/captures/manifest.json` | `logs/diff/osmu-four-room-flow-20260914-2208/captures/` | https://playwright.dev/docs/next/test-snapshots | https://playwright.dev/docs/emulation | https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/
