...
STAMP
line: osmu
artifact: 네 방 기본 흐름 QA v21
created_at: 2026-09-17 10:18 KST
...
benchmark: https://playwright.dev/docs/actionability, https://www.w3.org/WAI/WCAG22/Understanding/reflow.html
...
localhost 기능 범위는 PASS다. HEAD `2280089f`와 일치하는 실행본에서 생성실 후보 3장부터
...
| 항목 | 판정 | 증거 |
|---|---|---|
| 실행본 귀속과 health | PASS | `GET /api/health` HTTP 200, DB up, `build_commit=2280089f42130bd1c03f3116169bd11228bc166b`, 현재 HEAD 일치. `commands/13-curl-health-major-api.txt` |
| 백엔드 기본 흐름 | PASS | 최초와 seed 후 최종 실제 요청 11/11. 후보 3장, 편집 상태 변경, 발행 큐 HTTP 201, 제안 3건, 생성 큐 재인계. `commands/10-verify-basic-flow-final.txt` |
| 네 방 단면 | PASS | 최초와 seed 후 최종 4/4. 가린 모달 0, 401 0, 콘솔 오류 0. `commands/11-probe-four-room-final.txt` |
| 사람 클릭 네 폭 | PASS | 390 라이트·다크, 768, 1024, 1440에서 생성실→편집실→발행실→성과실 클릭 20/20, 성과실→생성실 5/5. 가로 넘침, 탐색 가림, 모달, 401, 콘솔 오류 0. `commands/12-verify-four-room-ui-final.txt`, 원본 `captures-final/` |
...
| schema, seed, RLS | PASS | `.env.local` 주입 후 `apply-schema.sh --seed` 멱등 적용. pgcrypto, pg_trgm, osmu_service 확인. `commands/08-apply-schema-seed-rls.txt` |
| 주요 API curl | PASS | health, metrics, drafts HTTP 200. 비밀값 없이 최상위 키와 건수만 기록. `commands/13-curl-health-major-api.txt` |
| QA 토큰 정리 | PASS | 활성 `qa-four-room-*` 토큰 0건, 최신 6/6 폐기. `commands/14-token-cleanup.txt` |
| 디자인 lint | PASS | 8pt 밖 임의 px, 인라인 style, 토큰 밖 hex 위반 0. `commands/09-design-lint.txt` |
| 릴레이 품질 게이트 | FAIL | stage 또는 운영 호스트 접촉 증거가 없어 `verify-agent-quality.sh`가 로컬 QA 출고를 반려했다. 이번 과제의 명시 범위는 localhost이고 운영 배포는 미검증이다. `commands/15-verify-agent-quality.txt` |
...
`logs/diff/osmu-four-room-flow-20260917-v21/` 아래에 있다. 동적 로컬 URL은
`http://localhost:3456`이다. 실제 운영 배포 버전은 미확인이다.

Playwright의 actionability 기준대로 보이고, 안정되고, 이벤트를 받을 수 있는 링크를 실제 클릭했다.
W3C Reflow 기준의 정보와 기능 손실 및 양방향 스크롤 여부를 `documentWidth`와 다음 행동 표시로
확인했다.

## 디자인 정합 행렬

기준은 v63 HTML과 `docs/design/prototypes/legacy-prototype-20260912/prototype/qa-v63/` PNG다.
현재 원본은 v21 `captures-final/`이다. 390 다크는 동작과 대비 확인에 포함했고, 행렬은
...
|---|---|---|---|---|
| R08 | 사이드바에서 네 방을 잇는다 | FLOW-UI-V21 | PASS | 네 방 4개, 네 폭, 성과실에서 생성실 복귀 5/5 |
| R19 | 지원 viewport에서 실제로 눌러 본다 | FLOW-UI-V21 | PASS | 390, 768, 1024, 1440과 390 다크 총 20화면 |
| R166, R172 | 생성부터 성과 재인계까지 기본 흐름을 먼저 살린다 | FLOW-API-V21 | PASS | 실제 요청 최종 11/11 |
| R193, R205, R206 | 승인 시안 계승과 화면 충실도 | DESIGN-CONF-V21 | NG | 16개 화면 모두 배치 속성 불일치 또는 동일 상태 미확보 |
| R207 | 성과실 UX와 학습 정보를 구성한다 | FLOW-PERF-V21 | 부분 PASS | 방향 제안 3건과 생성 큐 인계는 동작. v63 시각 정합 NG |
...
제품 화면, API, DB schema와 생성 계약을 변경하지 않았다. 이번에는 최신 HEAD에서 기존 검증기를
그대로 실행하고 증거만 새로 고정했다. 제품 소스 수정과 배포는 없다.
...
NG를 분리했고, 현재 HEAD와 실행 build SHA가 같은 것도 health 응답으로 고정했다.
...
SOURCES: `docs/design/prototypes/legacy-prototype-20260912/prototype/openclaw-auto-4room-v63.html` | `docs/design/prototypes/legacy-prototype-20260912/prototype/qa-v63/` | `docs/_archive/legacy-20260912/requests/회장-확정-요구사항-대장.md` | `wiki/2-product/build/사업좌표-OSMU와-ZERO-ONE.md` | `pipeline-state.osmu.md` | `docs/design/captures/manifest.json` | https://playwright.dev/docs/actionability | https://www.w3.org/WAI/WCAG22/Understanding/reflow.html
