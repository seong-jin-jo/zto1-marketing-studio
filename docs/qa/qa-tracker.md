## 2026-09-24 23:48 KST · 운영 생성기 인증 실패 관측 착수 ❌ NG

| 요청번호 | 요청 요지 | 테스트번호 | 판정 | 증거 |
|---|---|---|---|---|
| GEN-AUTH-OBS | 로컬에서 유효한 생성기 자격증명이 운영 컨테이너에서 거절되는 실제 오류를 확보 | GEN-AUTH-OBS-01~02 | ❌ NG, 관측 패치 중 | 배포 run `36012777008`은 자격증명 재배치 직후에도 로그인이 통하지 않았지만 `higgsfield auth token`의 stdout·stderr를 모두 폐기해 원인 문구가 없다. 실제 오류를 비밀값 가림 뒤 남기는 워크플로 회귀 테스트를 추가한다. |
