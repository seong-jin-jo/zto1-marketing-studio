...
|---|---|---|---|---|
| R68, API 갭 P2 | 두 갭 감사에서 지금도 없는 기본 흐름 항목 하나 구현 | GAP-HISTORY-20260922-1913-01 | NG | 현재 schema와 migration에는 게시물별 성과 관측 이력이 없다. 지정 작업 공간 `GET /api/metrics`는 HTTP 200, 키 `posts`, `coverage`, 게시물 0건이며 `history`, `comparison`은 없다. |
| 승인 범위 | pipeline에서 허용된 build 범위만 수정 | GAP-HISTORY-20260922-1913-02 | BLOCK | 현재 build는 콘텐츠 품질 1단계다. 승인 FDD §1.2가 발행실과 성과실 변경을 범위 밖으로 둔다. 성과 시계열의 관측 단위, 멱등 키, 보존 기간, 정규화, 비교식과 표본 기준도 미승인이다. 제품 소스 변경은 0건이다. |
| 실행본 귀속 | localhost 실행본과 현재 제품 소스 | GAP-HISTORY-20260922-1913-03 | 확인 | health HTTP 200, DB up, 실행 `5d112dfc`다. 실측 당시 HEAD와 일치했고 이후 제품 소스가 아닌 증거 문서만 커밋했다. |
| 전체 단위 및 통합 | `npm run test` | GAP-HISTORY-20260922-1913-04 | PASS | 378파일, 2,443건 통과, 3건 제외, 종료 코드 0이다. |
| TypeScript | `npx tsc --noEmit` | GAP-HISTORY-20260922-1913-05 | 조건부 PASS | 작업 트리는 `.next/dev/types` 파손으로 종료 코드 2다. `.next`를 제외하고 루트 상대 import를 함께 복제한 동일 소스에서는 같은 명령 종료 코드 0이다. |
| 디자인 토큰 | `design-lint.sh src` | GAP-HISTORY-20260922-1913-06 | PASS | 종료 코드 0, 위반 0이다. |
| 기본 흐름 실앱 | 생성, 편집, 발행 큐, 성과 재인계 | GAP-HISTORY-20260922-1913-07 | NG | localhost 첫 생성이 후보 0장과 `STUDIO_LLM_PROVIDER_UNAVAILABLE`, 종료 코드 1이다. |
| Studio v1 실앱 | 인증, 입력 거절, 생성과 조회 | GAP-HISTORY-20260922-1913-08 | NG | 401, 400, 422 거절 3건은 통과했다. 정상 생성은 기대 201 대신 HTTP 200 공급자 오류로 종료 코드 1이다. |

현재 판정은 BLOCK이다. 성과 시계열 기술설계 승인과 공유 생성 공급자 회복 뒤 같은 실행본에서 두 E2E를 다시 통과하기 전에는 구현 완료나 QA 전환으로 기록하지 않는다.
