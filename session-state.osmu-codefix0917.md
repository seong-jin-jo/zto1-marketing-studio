...
- 별도 API 읽기 전수 실사는 localhost에서 105개 경로와 106개 요청을 확인해 정상 88건,
  계약상 거절 18건, HTTP 500 0건을 관찰했다. 다만 격리 작업공간의 전체 테스트는
  `proper-lockfile` 내부 모듈을 찾지 못해 3건 실패했고 최종 보고 없이 종료됐다. 종료된 등록은 해제했다.
