...
- 현재 `pipeline-state.osmu.md`는 `qa`, `in-progress`, 승인 아님이라 신규 build가 허용되지 않는다.
- 게시물별 관측 단위, 멱등 키, 보존 기간, 공급자 정규화, 최근 30일과 직전 30일 비교식, 표본 부족 기준의 승인된 기술설계가 없다.
- 사용자 지정 v63과 pipeline 승인 v68 디자인 핀이 충돌한다. 이번 비화면 회수에서 임의 선택하지 않았다.
