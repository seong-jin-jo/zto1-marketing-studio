## 2026-09-23 12:1x 정정: 조용한 실패 잔존 아님. PR #82 정상 동작 확인

- 배포 환경 재실측(상호작용 수치): BODY_BEFORE=3190 → 숏폼 영상 만들기 클릭 → MODAL_PRESENT=1, MODAL_TEXT_LEN=228, MODAL_BTNS=["그만두기","이 범위 안에서 만들기"] → 승인 클릭 → MODAL_AFTER=0, NET=[[200,/api/studio/estimate,302바이트],[503,/api/higgsfield/image,108바이트]], BODY_AFTER=3396.
- 화면에 실제로 노출되는 문구: "이미지: 이미지 생성기에 로그인되어 있지 않습니다. 서버에서 생성기 로그인을 한 번 해 주시면 바로 쓰실 수 있습니다." **조용한 실패 없음. PR #82 가 의도대로 동작한다.**
- 직전 항목에서 "503 이 화면에 안 뜬다"고 적은 것은 컨트롤러의 좁은 정규식 오판이었다. 실수원장 substring-false-negative. 잔여 결함 없음으로 정정.
- 남은 차단 요인은 하나: 서버의 이미지 생성기 계정 미로그인(GENERATOR_UNAUTHENTICATED). 회장 실행 필요.

## 2026-09-23 11:5x 영상이 안 만들어지는 진짜 원인 확정: 이미지 생성기 미로그인

- PR #82(숏폼 영상 조용한 실패 제거) 컨트롤러 직접 검증 후 머지·배포. 기준선 대비 회귀 0(기준선 20 실패 = 병합본 20 실패, 로컬 환경 결함). typecheck:ci exit 0. 배포 run 35810038463 success, 운영 build_commit 40de32ee 확인.
- 배포 후 운영에서 "숏폼 영상 만들기" 클릭 → 비용 승인 창 정상 노출(300원~5,700원, 60~300초) → "이 범위 안에서 만들기" 승인 → **`/api/higgsfield/image` 503** 반환.
- 503 본문: {"error":"이미지 생성기에 로그인되어 있지 않습니다. 서버에서 생성기 로그인을 한 번 해 주시면 바로 쓰실 수 있습니다.","code":"GENERATOR_UNAUTHENTICATED"}
- **결론: 영상이 한 번도 안 만들어진 진짜 원인은 코드가 아니라 서버의 이미지 생성기 계정 미로그인이다.** 쇼츠·릴스·틱톡 발행이 계속 0 인 뿌리.
- **잔여 결함**: 그 503 이 화면에 한 글자도 안 뜬다(화면 문구 매칭 결과 0건). PR #82 가 try/catch 로 감쌌지만 ok:false 응답은 예외를 던지지 않아 토스트가 안 난다. ADR-007 조용한 실패가 이 경로에 남아 있다. 후속 수정 필요.
- 앞서 "대표 이미지 생성 성공(자산 6→33)"이라 보고한 것은 근거가 약하다. 자산 증가가 9장 덱 렌더 때문일 수 있다. 미검증으로 내린다.
